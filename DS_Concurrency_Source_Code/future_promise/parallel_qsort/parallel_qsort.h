...
namespace sorting
{
    ...
    template<class _Iter>
    void lquicksort(_Iter first, _Iter last)
    {
        lqsort(
            first,
            last,
    ...
    template<class _Iter, class _Compare>
    void pqsort(_Iter start, _Iter end, _Compare compare)
    {
        if ((end - start) > 10000)
        {
            ...
            // sort on own thread
            auto left_result = std::async(pqsort<_Iter, _Compare>, start, pivot, compare);            

            // continue on this thread
            pqsort(pivot + 1, end, compare);

            left_result.wait();
        }
        else
        {
            lqsort(start, end, compare);
        }
    }

    template<class _Iter, class _Compare>
    void lqsort(_Iter start, _Iter end, _Compare compare)
    {
        if ((end - start) > 1)
        {
            _Iter pivot = part(start, end, compare);

            lqsort(start, pivot, compare);
            lqsort(pivot + 1, end, compare);
        }
    }
}