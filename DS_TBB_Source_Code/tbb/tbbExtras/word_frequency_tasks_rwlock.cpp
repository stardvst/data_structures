...
namespace task_rw {
    ...
    class word_frequency {
        ...
        tbb::spin_rw_mutex frequency_mutex;

    public:
        void add(std::string word) {
            tbb::spin_rw_mutex::scoped_lock lock(frequency_mutex, true);
            ++frequency[word];
        }

        void get_top(int count, std::vector<frequency_pair>& trimmed) {

            tbb::spin_rw_mutex::scoped_lock lock(frequency_mutex);
